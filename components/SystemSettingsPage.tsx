...
import { Layers, Settings, Tag, Bell, CalendarDays, Users, type LucideIcon } from "lucide-react";
...
import TeamScheduleNotifyPage from "@/components/TeamScheduleNotifyPage";

// Phase 6-2J: 근태 항목/정책/코드 룩업 3개 페이지를 탭 통합. 기본 탭은 categories.
// (calendar 탭은 calendar_sources를 calendar-syncer가 자동 관리하므로 UI 제거)

type SubTab = "categories" | "policies" | "lookups" | "notifications" | "annual" | "teamSchedule";
...
const TABS: { id: SubTab; label: string; icon: LucideIcon }[] = [
  ...
  { id: "teamSchedule", label: "팀 일정 알림", icon: Users },
];
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      ...
      <div>
        ...
        {active === "teamSchedule" && <TeamScheduleNotifyPage />}
      </div>
    </div>
  );
}
